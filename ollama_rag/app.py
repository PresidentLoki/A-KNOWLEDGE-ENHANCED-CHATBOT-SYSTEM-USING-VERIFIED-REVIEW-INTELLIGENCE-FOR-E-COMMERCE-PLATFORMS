...
with st.sidebar:
    ...
    # Load configuration for embedding and main model
...
# Connect to local chroma DB client
import chromadb
chroma = chromadb.HttpClient(host="localhost", port=8000)
collection = chroma.get_or_create_collection("buildragwithpython")

# Function for generating LLaMA2 response using local Ollama model
def generate_llama2_response(prompt_input):
    # Generate query embedding using Ollama embeddings
    queryembed = ollama.embeddings(model=embedmodel, prompt=prompt_input)['embedding']

    # Retrieve relevant documents from chroma collection
    relevantdocs = collection.query(query_embeddings=[queryembed], n_results=5)["documents"][0]
    docs = "\n\n".join(relevantdocs)

    # Construct the query to the main model
    modelquery = f"""
    ...
    # Stream the response from Ollama Llama2 model
    response = ""
